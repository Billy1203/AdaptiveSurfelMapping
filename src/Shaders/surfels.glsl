...
float getRadius(float depth, float norm_z)
{
    ...
    float radius_n = radius / abs(norm_z);
...
float confidence(float maxRadDist2, float x, float y, float weighting)
{
    const float twoSigmaSquared = 0.72; //2*(0.6^2) from paper
    
    vec2 pixelPosCentered = vec2(x, y) - cam.xy;
    
    float radialRatio2 = dot(pixelPosCentered, pixelPosCentered) / maxRadDist2;
    
    return exp(-(radialRatio2 / twoSigmaSquared)) * weighting;
}
